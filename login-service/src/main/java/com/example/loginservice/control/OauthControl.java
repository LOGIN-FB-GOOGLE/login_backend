...
public class OauthControl {
    ...
    Response google(@RequestBody TokenDTO tokenDTO) throws IOException {
        ...
        if (userRepository.existsByEmail(payload.getEmail())){
            entity = userRepository.findByEmail(payload.getEmail()).get();
        }else {
            if (Objects.nonNull(userService.registerUser(payload.getEmail()))){
    ...
    public @ResponseBody Response facebook(@RequestBody TokenDTO tokenDTO) throws IOException {
        Facebook facebook = new FacebookTemplate(tokenDTO.getValue());
//        final String[] fields = {"email", "picture"};
        User user = facebook.fetchObject("me", User.class);
        com.example.loginservice.entity.User entity = new com.example.loginservice.entity.User();
        if (userRepository.existsByEmail(user.getEmail())){
            entity = userRepository.findByEmail(user.getEmail()).get();
        }else {
            if (Objects.nonNull(userService.registerUser(user.getEmail()))){
